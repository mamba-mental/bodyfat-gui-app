...
# Local imports
from .PRIME_Utils import calculate_age
from .PRIME_Calculations import calculate_lean_mass_preservation_scores
from .PRIME_AI_Analyzer_Factory import create_ai_analyzer_sync
...
def create_professional_charts(progression_data, output_dir):
    ...
    Saves PNG files to disk and returns both Base64 data (for HTML) and file paths (for Markdown).
    ...
    Returns:
        dict: Chart data containing 'base64' strings and 'paths' to files
    """
    ...
    try:
        ...
        weight_chart_filename = f"weight_progress_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    ...
    try:
        ...
        for ax in [ax1, ax2]:
            ax.tick_params(axis='x', rotation=45)
        plt.tight_layout()
        body_comp_filename = f"body_composition_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
...
def generate_prime_report_terminal_fast(user_data, progression_data, output_dir="results"):
    """
    Generate a comprehensive PRIME report with configurable AI analysis.
    ...
    # Calculate comprehensive metrics needed for AI context
    ...
    protein_intake = user_data.get('protein_intake', 150)
    ...
    try:
        print("[INFO] Initializing AI Analyzer...")
        analyzer = create_ai_analyzer_sync()
        if analyzer:
            print(f"[INFO] AI Analyzer available ({analyzer.provider}/{analyzer.model}), running confidence analysis...")
            input_analysis = analyzer.analyze_input_parameters(user_data)
            calculation_results = {
                'tdee': initial_tdee,
                'daily_calorie_intake': progression_data[0]['daily_calorie_intake'],
                'projected_weight_loss': total_weight_loss,
                'projected_fat_loss': total_fat_loss,
                'projected_muscle_gain': muscle_change,
                'metabolic_adaptation': metabolic_adaptation,
                'total_weeks': total_weeks,
                'muscle_retention_rate': muscle_retention_rate
            }
            confidence_analysis = asyncio.run(analyzer.generate_ai_confidence_analysis(user_data, calculation_results))
            print("[INFO] AI Analysis complete.")
        else:
            print("[INFO] AI Analyzer unavailable (no config found), using defaults.")
    except Exception as e:
    ...
    context = {
        ...
        'ai_analysis_note': '' if confidence_analysis else 'AI confidence analysis skipped or unavailable.'
    }
    # Load template
    ...
    if template_path is None or not os.path.exists(template_path):
        raise FileNotFoundError(f"Report template not found. Looked for {candidate_files} or report*.html in: {templates_dir}")
...
{f"**AI Analysis Confidence:** {confidence_analysis.overall_score:.1f}/100" if confidence_analysis else "*AI analysis unavailable*"}
